...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Normal Mapping", NULL, NULL);
	if (window == NULL)
	{
	...
	TexLoader wallTex("texture/brickwall.jpg", GL_REPEAT, GL_LINEAR, GL_LINEAR, true);
	GLint wallTexID = wallTex.GetMapID();
	TexLoader normTex("texture/brickwall_normal.jpg", GL_REPEAT, GL_LINEAR, GL_LINEAR, false); // remember not to sRGB correction normal map
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightShader.UseProgram();
		lightShader.SetUniform1i("useNormalMap", useNormalMap);
