...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Stencil Testing", NULL, NULL);
	if (window == NULL)
	{
	...
	GLSLloader singleColor("shader/vertex.glsl", "shader/fragSingleColor.glsl");
	singleColor.CreateCompile();
	singleColor.CreateProgram();
	ModelLoader ourModel("model/cube/cube.obj");

	TexLoader floorTex("texture/Tileable_marble.jpg");
	unsigned int floorTexID = floorTex.GetMapID();
	GLint texLoc = glGetUniformLocation(planeShader.GetProgram(), "texture_diffuse1");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// render the loaded model outline
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		glStencilMask(0x00);
		glDisable(GL_DEPTH_TEST);
		singleColor.UseProgram();
		model = glm::mat4();
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.1f, 1.1f, 1.1f));
		singleColor.SetUniformMat4("projection", projection);
		singleColor.SetUniformMat4("view", view);
		singleColor.SetUniformMat4("model", model);
