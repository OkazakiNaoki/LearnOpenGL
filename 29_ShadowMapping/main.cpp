...
int main()
{
	...
	unsigned int planeVAO, planeVBO, cubeVAO, cubeVBO, quadVAO, quadVBO;
	...
	GLSLloader simpleDepthShader("shader/vertexSimpleDepth.glsl", "shader/fragSimpleDepth.glsl");
	simpleDepthShader.CreateCompile();
	simpleDepthShader.CreateProgram();

	GLSLloader debugDepthShader("shader/vertDebug.glsl", "shader/fragDebug.glsl");
	debugDepthShader.CreateCompile();
	debugDepthShader.CreateProgram();
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, shadowWidth, shadowHeight, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL); //format as GL_DEPTH_COMPONENT
	...
	glDrawBuffer(GL_NONE); //framebuffer can not work without color buffer, so we need to explicitly specify not to use color buffers
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		simpleDepthShader.SetUniformMat4("lightSpaceMatrix", lightSpaceMatrix);
		...
		// plane part
		glm::mat4 model;
		simpleDepthShader.SetUniformMat4("model", model);
		glBindVertexArray(planeVAO);
		...
		// visualize
		debugDepthShader.UseProgram();
		debugDepthShader.SetUniform1f("near_plane", 1.0f);
		debugDepthShader.SetUniform1f("far_plane", 7.5f);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, depthMap);
		glBindVertexArray(quadVAO);
