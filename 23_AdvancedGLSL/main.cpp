...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Advanced GLSL", NULL, NULL);
	if (window == NULL)
	{
	...
	//*********************************************
	//	Create four shader to use same project,view matrix
	//*********************************************
	GLSLloader redShader("shader/vertex.glsl", "shader/fragRed.glsl");
	redShader.CreateCompile();
	redShader.CreateProgram();
	GLSLloader greenShader("shader/vertex.glsl", "shader/fragGreen.glsl");
	greenShader.CreateCompile();
	greenShader.CreateProgram();
	GLSLloader blueShader("shader/vertex.glsl", "shader/fragBlue.glsl");
	blueShader.CreateCompile();
	blueShader.CreateProgram();
	GLSLloader yellowShader("shader/vertex.glsl", "shader/fragYellow.glsl");
	yellowShader.CreateCompile();
	yellowShader.CreateProgram();
	//*********************************************
	//	Get uniform block index of that shader
	//*********************************************
	unsigned int uniformBlockIndexRed = glGetUniformBlockIndex(redShader.GetProgram(), "Matrices");
	unsigned int uniformBlockIndexGreen = glGetUniformBlockIndex(greenShader.GetProgram(), "Matrices");
	unsigned int uniformBlockIndexBlue = glGetUniformBlockIndex(blueShader.GetProgram(), "Matrices");
	unsigned int uniformBlockIndexYellow = glGetUniformBlockIndex(yellowShader.GetProgram(), "Matrices");
	//*********************************************
	//	Then bind that index to the binding point (or you can bind in shader program)
	//*********************************************
	...
	glGenBuffers(1, &ubo);
	//*********************************************
	//	Bind and allocate size (not yet assign data)
	//*********************************************
	glBindBuffer(GL_UNIFORM_BUFFER, ubo);
	glBufferData(GL_UNIFORM_BUFFER, 2 * sizeof(glm::mat4), NULL, GL_STATIC_DRAW);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
	//*********************************************
	//	Now bind that ubo to binding point
	//*********************************************
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//*********************************************
		//	Feed data now by glBufferSubData
		//*********************************************
		glBindBuffer(GL_UNIFORM_BUFFER, ubo);
		...
		yellowShader.SetUniformMat4("model", model);
		glDrawArrays(GL_TRIANGLES, 0, 36);
