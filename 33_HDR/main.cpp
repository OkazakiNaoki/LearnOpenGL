...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "HDR", NULL, NULL);
	if (window == NULL)
	{
	...
	GLSLloader hdrShader("shader/vertHDR.glsl", "shader/fragHDR.glsl");
	hdrShader.CreateCompile();
	hdrShader.CreateProgram();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightShader.SetUniform1i("inverse_normals", true);
		renderCube(&cubeVAO, &cubeVBO);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);

		//*********************************************
		//	16bit RGBA Quad render
		//*********************************************
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		hdrShader.UseProgram();
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, colorBuffer);
		hdrShader.SetUniform1i("hdr", mode);
		...
		std::cout << "\33[2K\r";
		std::cout << "HDR : " << showMode << " , exposure : " << exposure;
